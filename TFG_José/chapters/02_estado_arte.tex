% chapters/02_estado_arte.tex
\chapter{Estado del arte y fundamentos de la optimización basada en modelos sustitutos}
...
% QUIZAS MERECE LABEL. PUEDE QUE LA USEMOS EN EXPERIMENTOS.
\[
...
\begin{figure}[htbp]
    ...
    \caption{(a) Muestras de funciones generadas por el prior del GP. (b) Distribución posterior tras condicionar en dos observaciones; la línea negra representa la media predictiva y la banda sombreada el intervalo predictivo del 95 \%.}
...
% Comentario indicaba que esta sección se podía resumir.
...
\begin{itemize}
    ...
    especie. Al igual que en el kernel SE, \(\sigma_f^2\) controla la amplitud de la función y
...
En el caso de los procesos gaussianos, esta idea adopta una forma particular. Como se ha
visto en la Sección~\ref{subsec:truco_del_kernel}, los pesos \(\bm{w}\) de la representación en
espacio de características quedan marginalizados y su efecto pasa a estar codificado en el
kernel. Por tanto, el entrenamiento de un GP no consiste en ajustar directamente un vector
de pesos \(\bm{w}\), sino en seleccionar los parámetros \(\bm{\theta}\) que definen la función
de covarianza y el modelo de ruido. Entre ellos se encuentran, por ejemplo, la longitud
característica \(\ell\), la varianza de señal \(\sigma_f^2\) y la varianza del ruido
\(\sigma_n^2\).
...
Sección~\ref{subsec:truco_del_kerneld}, los pesos \(\bm{w}\) de la representación en
espacio de características quedan marginalizados y su efecto pasa a estar codificado en el
...
diversos criterios, como \textit{Probability of Improvement} (PI), \textit{Expected Improvement}
(EI) o \textit{Lower Confidence Bound} (LCB), en este trabajo nos centraremos en el
\textit{Expected Improvement}, uno de los criterios más utilizados en BO
...
\begin{figure}[h]
    ...
    \caption{(a) Media predictiva del GP condicionada a las observaciones disponibles. 
...
\subsection{Métricas probabilísticas (NLL, NLPD)} %  y cobertura del 95 \% SI LA ACABO PONIENDO
...
%\paragraph{Cobertura de intervalos del 95 \%.}

%La cobertura de intervalos es una métrica empírica sencilla para comprobar si los intervalos
%predictivos del modelo tienen una frecuencia de acierto coherente con su nivel nominal. Para
%una distribución normal, aproximadamente el 95 \% de la masa de probabilidad se encuentra
%dentro de \(1.96\) desviaciones típicas respecto a la media. Por ello, se define el intervalo
%predictivo
%\[
%I_i^{95}
%=
%[\mu_i-1.96s_i,\ \mu_i+1.96s_i].
%\]
%La cobertura empírica al 95 \% se calcula como
%\begin{equation}
%\mathrm{Cov}_{95}
%=
%\frac{1}{N}
%\sum_{i=1}^{N}
%\mathbb{I}(y_i\in I_i^{95}),
%\label{eq:coverage95}
%\end{equation}
%donde \(\mathbb{I}(\cdot)\) es la función indicatriz, que vale \(1\) si la condición se cumple
%y \(0\) en caso contrario.

%Un modelo bien calibrado debería obtener una cobertura cercana a \(0.95\). Si
%\(\mathrm{Cov}_{95}\) es considerablemente menor que \(0.95\), las observaciones reales caen
%fuera de los intervalos más a menudo de lo esperado, lo que indica sobreconfianza. En ese
%caso, podría ser necesario revisar la estimación del ruido \(\sigma_n^2\), probar kernels menos
%restrictivos, ajustar mejor los parámetros o aumentar la cantidad de datos en regiones
%donde el modelo falla.

%Si, por el contrario, \(\mathrm{Cov}_{95}\) es cercana a \(1\), el modelo contiene prácticamente
%todas las observaciones dentro de sus intervalos. Esto puede parecer positivo, pero suele
%indicar subconfianza si los intervalos son demasiado anchos. En ese caso, conviene revisar si
%la varianza predictiva está siendo sobreestimada, si el término de ruido es excesivo o si el
%kernel elegido está generando un modelo demasiado conservador.

%La cobertura, por sí sola, no mide la precisión de las predicciones ni la anchura de los
%intervalos. Por esta razón, debe interpretarse junto con métricas como NLPD, RMSE o MAE:
%un buen modelo probabilístico debe producir predicciones precisas, intervalos razonablemente
%estrechos y una cobertura coherente con el nivel nominal.